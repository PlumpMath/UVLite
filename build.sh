...
if [ $OS = $PLATFORM_WINDOWS ]; then
    ...
    echo "----------------------------------------"
    $GYP --depth=. -Icommon.gypi -Dlibrary=static_library -Duv_library=static_library -Dtarget_arch=x64 --build=$CONFIGURATION UVLite.gyp
    msbuild /p:Configuration=$CONFIGURATION UVLite.sln
else
    # Compiling wrk
    echo "----------------------------------------"
    echo "Compiling wrk"
    ...
    echo "----------------------------------------"
    $GYP --depth=. -Goutput_dir=./builds/unix -Icommon.gypi -Dlibrary=static_library -Duv_library=static_library -Dtarget_arch=x64 --build=$CONFIGURATION -f make UVLite.gyp
fi
